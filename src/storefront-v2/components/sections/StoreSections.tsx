...
export function StoreSections({ 
  ...
  // Group products by sections if products are provided
  const showcaseProducts = products?.slice(0, 2) || [];
  const availableProducts = products?.slice(2, 5) || [];
  const promoProducts = products?.slice(5, 7) || [];
  const catalogProducts = products?.slice(7, 11) || [];
  
  return (
    <div className="space-y-6 lg:space-y-8">
      <CatalogSection products={catalogProducts} onAddToCart={onAddToCart} cartItems={cartItems} onQuickView={onQuickView} />
      <ShowcaseSection products={showcaseProducts} onAddToCart={onAddToCart} cartItems={cartItems} onQuickView={onQuickView} />
      <AvailableSection products={availableProducts} onAddToCart={onAddToCart} cartItems={cartItems} onQuickView={onQuickView} />
      <PromoSection products={promoProducts} onAddToCart={onAddToCart} cartItems={cartItems} onQuickView={onQuickView} />
      <StoreInfoSection />
      <ReviewsSection />
      <Footer />
    </div>
