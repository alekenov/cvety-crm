...
function StoreInfo({ shopInfo }: { shopInfo?: ShopInfo | null }) {
  const name = shopInfo?.name || 'Cvety.kz';
  const rating = shopInfo?.rating || 4.6;
  const reviewsCount = shopInfo?.reviews_count || 827;
  const deliveryPrice = shopInfo?.delivery_price || 2000;
  const deliveryTime = shopInfo?.delivery_time || '2-4 часа';
  const pickupAddress = shopInfo?.pickup_address || 'мкр. Самал-2, 111';
    ...
    <div className="flex items-start gap-3 lg:gap-4">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1 lg:mb-2">
          <h1 className="text-gray-900 truncate">{name}</h1>
        ...
        <div className="hidden lg:flex lg:items-center lg:gap-3 lg:text-xs">
          <span className="text-gray-600">Доставка от {deliveryPrice} ₸ • По Алматы</span>
          <span className="font-medium text-red-600">{deliveryTime}</span>
          <span className="text-gray-400">•</span>
          <span className="text-gray-600">Самовывоз • {pickupAddress}</span>
