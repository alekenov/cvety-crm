...
export function MobileProductList({ products, onUpdateQuantity, onUpdateProduct, onShowProductDetail }: MobileProductListProps) {
  ...
  const getStockBadge = (product: Product) => {
    if (product.quantity === 0) {
      return <Badge className="bg-muted text-muted-foreground px-3 py-1">Нет</Badge>;
    }
    return null;
  };
  ...
  return (
    <div className="space-y-4">
      {products.map((product) => (
        <Card key={product.id} className={`p-4 ${getCardClass(product)}`}>
          <div className="space-y-4">
            {/* Header */}
            <div className="flex items-start justify-between">
              <div className="flex-1 min-w-0">
                {editingId === product.id ? (
                  ...
                  >
                    <h3 className={`leading-tight hover:text-primary transition-colors ${getTextClass(product)}`}>
                      {product.name}
                    </h3>
                    <p className={`text-sm text-muted-foreground mt-1 ${getTextClass(product)}`}>{product.category}</p>
                    ...
                    {product.discount && product.discount > 0 && (
                      <p className="text-sm text-primary mt-1">Скидка {product.discount}%</p>
                    )}
                  </div>
                )}
              </div>
            ...
            <div className="flex items-center justify-between">
              {editingId === product.id ? (
                ...
                <span className={getTextClass(product)}>₸{product.price}</span>
              )}
              ...
              <div className="flex items-center gap-3">
                ...
                <span className={`min-w-[3rem] text-center ${getTextClass(product)}`}>
                  {product.quantity}
                </span>
